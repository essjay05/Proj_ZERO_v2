...
passport.use('local-signup', new LocalStrategy({
    ...
    // CHECK TO SEE IF THERE'S A MATCHING EMAIL IN THE DATABASE ALREADY...
    // EMAIL WILL BE THE UNIQUE IDENTIFIER
    User.findOne({ email }, (err, user) => {
        if (err) return done(err);
        // IF EMAIL ALREADY TAKEN, RETURN FLASH MESSAGE
        if (user) return done(null, false, req.flash('signupMessage', 'That email is already taken.'));

        // IF NO ONE MATCHING EMAIL IN DATABASE FOUND... CREATE NEW USER :)
